...
describeExcludeTouch()('touch interactions', () => {
  ...
  beforeEach(function () {
    ...
    if (!target.hasOwnProperty('ontouchstart')) {
      // To support touchstart events
      target.ontouchstart = sinon.spy()
    }
    if (!target.hasOwnProperty('ontouchmove')) {
      // To support touchstart events
      target.ontouchmove = sinon.spy()
    }
    if (!target.hasOwnProperty('ontouchend')) {
      // To support touchstart events
      target.ontouchend = sinon.spy()
    }
